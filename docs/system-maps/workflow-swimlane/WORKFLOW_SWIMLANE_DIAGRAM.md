...
```mermaid
flowchart TD
    Start[الوثيقة في مرحلة Review1/Proofread] --> Review[المكلّف يراجع الوثيقة]
    Review --> Decision{قرار المراجعة}
    
    Decision -->|✅ موافقة| Approve[approve<br/>→ finalapproval]
    Decision -->|➡️ تحويل| Forward[forward<br/>→ المرحلة التالية]
    Decision -->|❌ رفض| Reject[reject<br/>→ draft]
    ...
    Activity1 --> Toast1[Toast: 'تم الموافقة']
    Activity2 --> Toast2[Toast: 'تم التحويل']
    Activity3 --> Toast3[Toast: 'تم الرفض']
    
    Toast1 --> End[نهاية]
    Toast2 --> End
    Toast3 --> Draft[الوثيقة تعود للمسودة]
